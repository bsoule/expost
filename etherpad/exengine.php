...
function preprocess($content)
{
  ...
  foreach(explode("\n", $content) as $x) {
    while(preg_match('/\$(REF|FN)\[([^\]]*)\]/', $x, $m)) {
      ...
      if($f==='FN')
        #$expost_fnt[$tag]++; # we were getting warnings in the logs with this
        $expost_fnt[$tag] = ($expost_fnt[$tag] ?? 0) + 1;
  ...
  foreach(explode("\n", $content) as $x) {
    while(preg_match('/\$([a-zA-Z]\w*)/', $x, $m)) {
      ...
      if(isset($expost_fnt[$tag])) {
        if (!isset($fc)) $fc = []; # adding this to avoid warnings in the logs
        #$fc[$tag]++; # we were getting warnings in the logs with this
        $fc[$tag] = ($fc[$tag] ?? 0) + 1;
...
$ep = $_REQUEST['pad'] ?? 'expost'; # change to prevent warnings in the logs
